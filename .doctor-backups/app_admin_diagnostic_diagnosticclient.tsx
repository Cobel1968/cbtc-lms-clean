...
export default function diagnosticclient() {
  const [assessments, set_assessments] = useState<any[]>([]);
  const [is_loading, set_is_loading] = useState(true);

  useEffect(() => {
    async function fetch_assessments() {
      const { data, error } = await supabase
        .from('vocational_assessments')
        .select('*')
        .order('created_at', { ascending: false });
      
      if (data) set_assessments(data);
      ...
      <div className="max-w-6xl mx-auto">
        <header className="mb-8 flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-500 text-sm">Feature 4: Analog-to-Digital Integrity Check</p>
          </div>
          <div className="bg-white px-4 py-2 rounded-lg shadow-sm border border-gray-200 text-sm font-bold text-blue-600">
                ...
                <div className="p-6">
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex gap-4 items-center">
                      ...
                      <div>
                        <div className="text-[10px] font-mono text-gray-400 uppercase">REF: {item.id.slice(0,8)}</div>
                  ...
                  <div className="grid md:grid-cols-3 gap-6 mt-6 pt-6 border-t border-gray-100">
                    <div>
                      <h4 className="text-[10px] uppercase font-bold text-gray-400 mb-2 flex items-center gap-1">
                        <Search size={12} /> Raw OCR Text
                      </h4>
                    ...
                    <div>
                      <h4 className="text-[10px] uppercase font-bold text-gray-400 mb-2 flex items-center gap-1">
                        <Languages size={12} /> Technical Mapping
                      </h4>
