    ...
    async function fetchDashboardData() {
      try {
        ...
        if (profile.role === 'admin') {
            console.log("[Cobel Engine] Admin context active.");
        }
        ...
        console.error("[Cobel Engine] Sync Error:", err.message);
        setError("synchronization error: check connection to cobel terminal");
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleHandwritingUpload = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Update mastery minutes locally to reflect AI extraction immediately
        setStats(prev => ({ 
          ...prev, 
      ...
      setError("bridge ingestion failed. verify image quality.");
